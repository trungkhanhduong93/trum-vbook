var DEFAULT_BASE = "https://nettruyenar.com";
...
function syncBaseFromUrl(url) {
    if (!url) return;
    var m = String(url).match(/^https?:\/\/((?:www\.)?nettruyen[a-z0-9-]*\.[a-z]+)/i);
    if (m) {
...
function swapDomainTo(url, targetDomain) {
    ...
    return String(url).replace(/^https?:\/\/(?:www\.)?nettruyen[a-z0-9-]*\.[a-z]+/i, targetDomain);
}
...
function parseItems(doc) {
    ...
    for (var i = 0; i < cards.size(); i++) {
        ...
        var chapEl = selFirst(card, ".comic-item li.chapter a, .chapter a, li.chapter a");
