...
function execute(url) {
    ...
    // Title
    var titleEl = selFirst(doc, "h1.title-detail, h1");
    var name = titleEl ? titleEl.text().trim() : "";

    // Cover
    var cover = "";
    var coverEl = selFirst(doc, ".detail-info .col-image img, .detail-info img, .col-xs-4 img");
    if (coverEl) {
    ...
    var rows = doc.select(".list-info li");
    ...
    // Genres
    var genres = [];
    var gLinks = doc.select(".list-info li.kind a, a[href*='/the-loai/']");
    var gSeen = {};
    ...
    // Description
    var description = "";
    var descEl = selFirst(doc, ".detail-content p, .detail-content, .shortened");
